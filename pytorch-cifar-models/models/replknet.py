...
def create_RepLKNet31T(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,12,2], channels=[48, 96, 192, 384],
                    drop_path_rate=drop_path_rate, small_kernel=5, num_classes=num_classes, use_checkpoint=use_checkpoint,
                    small_kernel_merged=small_kernel_merged)

def create_RepLKNet31S(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False):
...
if __name__ == '__main__':
    model = create_RepLKNet31S(small_kernel_merged=False)
    model.eval()
    stat(model, (3, 32, 32))
