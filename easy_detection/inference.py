...
LOAD_CHECKPOINT = '/remote-home/zhangjingdong/easy_detection/checkpoints/faster_rcnn_voc/13_Faster_RCNN.pt'
INFERENCE_LIST = 'datasets/voc_sample/ImageSets/Main/train.txt'
IMAGE_FOLDER = 'datasets/voc_sample/JPEGImages'
KEEP_THRESH = 0.5
...
def inference_rpn(bboxes, scores, image_path, savepath=None):
    """
    Args:
        image_path: image path
        keep_thresh: 置信度大于多少的保留
    ...
    # labels = labels[keep]
    # scores = scores[keep]
    labels = np.array(np.zeros(scores.shape[0]), dtype='uint8')
    scores = (scores) / (np.max(scores))
    bboxes = bboxes / 2.0

    R = lambda x: int(round(x, 0))  # 四舍五入
    visualize_boxes(image=image_org, boxes=bboxes, labels=labels, probs=scores, class_labels=['proposal'])
    ...
    if savepath:
        ...
        print("bbox:{}".format(bboxes))
        image = cv2.cvtColor(np.asarray(image_org), cv2.COLOR_RGB2BGR)
        cv2.imwrite(f'{savepath}/{filaname}_rp_all.png', image)
...
if __name__ == '__main__':
    ...
    for i, line in enumerate(lines):
        ...
        #inference_rpn(np.load('./{}_box.npy'.format(i+1)), np.load('./{}_score.npy'.format(i+1)), image_path, savepath=SAVE_PATH)
