...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            writer.add_scalar('train/loss', mloss[3], epoch)
            writer.add_scalar('train/giou', mloss[0], epoch)
            writer.add_scalar('train/loss_conf', mloss[1], epoch)
            writer.add_scalar('train/loss_cls', mloss[2], epoch)
            ...
            if epoch >= 0:
                ...
                for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes) in enumerate(self.test_dataloader):
                    with torch.no_grad():
                        ...
                        testloss, _, _, _ = self.criterion(p, p_d, label_sbbox, label_mbbox, label_lbbox,
                                                                              sbboxes, mbboxes, lbboxes)
                        testlosses.append(testloss)

                writer.add_scalar('test/testloss', torch.mean(torch.Tensor(testlosses)), epoch)
